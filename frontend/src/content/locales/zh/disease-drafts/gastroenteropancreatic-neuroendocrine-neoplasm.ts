...
export const diseaseGastroenteropancreaticNeuroendocrineNeoplasm: DiseaseDraft =
  {
    ...
    medicalSections: {
      symptoms:
        '<p>主要临床表现包括：GEP-NEN 分为无功能性肿瘤和有功能性肿瘤。</p><p>不同患者不一定会出现所有表现，症状轻重也可能随年龄、病程和合并问题而变化。</p>',
      diagnosis:
        '<p>诊断通常需要把病史、体格检查、专科检查和必要的实验室/影像/病理或遗传检测放在一起判断。GEP-NEN 确诊依赖组织病理。</p><p>还需要与表现相近的疾病鉴别：GEP-NEN 会和所在器官的其他类型肿瘤在临床表现上有相似之处，如 G-NEN 和胃腺癌、结直肠 NEN 和结直肠癌等。</p>',
      treatment:
        '<p>治疗应由相关专科根据疾病阶段、严重程度、受累器官和药物可及性制定。1.内科治疗内科治疗的目的是缓解功能性 GEP-NEN 激素分泌相关的临床症状或综合征以及控制肿瘤生长。</p>',
      longTermCare:
    ...
    symptoms: '<p>GEP-NEN 分为无功能性肿瘤和有功能性肿瘤。</p>',
    diagnosis: '<p>GEP-NEN 确诊依赖组织病理。</p>',
    treatment:
      '<p>内科治疗内科治疗的目的是缓解功能性 GEP-NEN 激素分泌相关的临床症状或综合征以及控制肿瘤生长。</p>',
    prognosis:
