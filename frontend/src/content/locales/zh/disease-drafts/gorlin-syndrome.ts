...
export const diseaseGorlinSyndrome: DiseaseDraft = {
  ...
  medicalSections: {
    symptoms:
      '<p>主要临床表现包括：本病包括 100 多种症状和体征，主要表现包括：①早期多发性 BCC；</p><p>不同患者不一定会出现所有表现，症状轻重也可能随年龄、病程和合并问题而变化。</p>',
    diagnosis:
      '<p>诊断通常需要把病史、体格检查、专科检查和必要的实验室/影像/病理或遗传检测放在一起判断。NBCCS 的诊断标准如下，满足以下一项标准者可以确定诊断：①符合 1 项主要标准和遗传学检查；</p><p>还需要与表现相近的疾病鉴别：NBCCS 的鉴别诊断包括一系列与 NBCCS 有部分共同临床特征的综合征：Sotos 综合征 1 型(又称 5q35 缺失综合征；</p>',
    treatment:
      '<p>治疗应由相关专科根据疾病阶段、严重程度、受累器官和药物可及性制定。NBCCS 患者需要多学科处理。</p>',
    longTermCare:
  ...
  symptoms:
    '<p>本病包括 100 多种症状和体征，主要表现包括：①早期多发性 BCC；</p>',
  diagnosis:
    '<p>NBCCS 的诊断标准如下，满足以下一项标准者可以确定诊断：①符合 1 项主要标准和遗传学检查；</p>',
  treatment: '<p>NBCCS 患者需要多学科处理。</p>',
  prognosis:
