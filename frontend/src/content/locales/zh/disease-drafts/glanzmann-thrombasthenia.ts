...
export const diseaseGlanzmannThrombasthenia: DiseaseDraft = {
  ...
  medicalSections: {
    symptoms:
      '<p>主要临床表现包括：GT 的典型临床表现为出血，最常见的是紫癜、鼻出血(60%～80%)、牙龈出血(20%～60%)、月经过多(60%～90%)。</p><p>不同患者不一定会出现所有表现，症状轻重也可能随年龄、病程和合并问题而变化。</p>',
    diagnosis:
      '<p>诊断通常需要把病史、体格检查、专科检查和必要的实验室/影像/病理或遗传检测放在一起判断。GT 诊断需综合临床表现和实验室检测结果进行分析确诊。</p><p>还需要与表现相近的疾病鉴别：本病应与其他血小板计数和形态正常的血小板功能缺陷型疾病相鉴别，常见的需要与GT 鉴别诊断的疾病包括：白细胞黏附缺陷Ⅲ 型白细胞黏附缺陷Ⅲ 型(LAD-Ⅲ )是一种常染色体隐性遗传病，由kindlin 3 基因 fermt3 的致病变异引起，表现为血小板、白细胞和内皮细胞中“ 由内而外” 整合素激活缺陷，导致出血、感染和伤口愈合受损。</p>',
    treatment:
      '<p>治疗应由相关专科根据疾病阶段、严重程度、受累器官和药物可及性制定。1.一般原则患者教育应侧重于口腔卫生管理，避免高风险运动和施加压力导致鼻出血。</p>',
    longTermCare:
  ...
  symptoms:
    '<p>GT 的典型临床表现为出血，最常见的是紫癜、鼻出血(60%～80%)、牙龈出血(20%～60%)、月经过多(60%～90%)。</p>',
  diagnosis: '<p>GT 诊断需综合临床表现和实验室检测结果进行分析确诊。</p>',
  treatment:
    '<p>一般原则患者教育应侧重于口腔卫生管理，避免高风险运动和施加压力导致鼻出血。</p>',
  prognosis:
