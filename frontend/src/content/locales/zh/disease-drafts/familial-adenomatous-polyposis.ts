...
export const diseaseFamilialAdenomatousPolyposis: DiseaseDraft = {
  ...
  medicalSections: {
    symptoms:
      '<p>主要临床表现包括：FAP 的临床表现包括肠内表现和肠外表现两大类。</p><p>不同患者不一定会出现所有表现，症状轻重也可能随年龄、病程和合并问题而变化。</p>',
    diagnosis:
      '<p>诊断通常需要把病史、体格检查、专科检查和必要的实验室/影像/病理或遗传检测放在一起判断。累计发现≥ 10 个结直肠腺瘤的患者均应考虑 FAP。</p><p>还需要与表现相近的疾病鉴别：常见的需要与 FAP 鉴别诊断的疾病包括：MUTYH 相关性息肉病（MUTYH associated polyposis，MAP）是常染色体隐性遗传综合征，由 MUTYH 基因突变所致，该基因属于碱基切除修复系统，可以保护基因组信息免受氧化应激损害。</p>',
    treatment:
      '<p>治疗应由相关专科根据疾病阶段、严重程度、受累器官和药物可及性制定。预防性全结肠或全结直肠切除手术是本病的治疗方式。</p>',
    longTermCare:
  ...
  symptoms: '<p>FAP 的临床表现包括肠内表现和肠外表现两大类。</p>',
  diagnosis: '<p>累计发现≥ 10 个结直肠腺瘤的患者均应考虑 FAP。</p>',
  treatment: '<p>预防性全结肠或全结直肠切除手术是本病的治疗方式。</p>',
  prognosis:
