...
export const diseaseSystemicMastocytosis: DiseaseDraft = {
  ...
  medicalSections: {
    symptoms:
      '<p>主要临床表现包括：SM 的临床表现包括全身症状、肥大细胞活化症状、相关疾病症状以及体征：（1）全身症状：发热、乏力、盗汗、体重下降等。</p><p>不同患者不一定会出现所有表现，症状轻重也可能随年龄、病程和合并问题而变化。</p>',
    diagnosis:
      '<p>诊断通常需要把病史、体格检查、专科检查和必要的实验室/影像/病理或遗传检测放在一起判断。SM 需满足主要标准和 1 项次要标准或同时满足≥3 项次要标准（表 75-3），其次再对SM 亚型进行诊断（图 75-1，图 75-2）。</p>',
    treatment:
      '<p>治疗应由相关专科根据疾病阶段、严重程度、受累器官和药物可及性制定。1.肥大细胞活化症状的抗介质治疗（1）避免诱因（食物、药物、过敏原、温度骤变、皮肤干燥、摩擦皮肤）；</p>',
    longTermCare:
  ...
  symptoms:
    '<p>SM 的临床表现包括全身症状、肥大细胞活化症状、相关疾病症状以及体征：（1）全身症状：发热、乏力、盗汗、体重下降等。</p>',
  diagnosis:
    '<p>SM 需满足主要标准和 1 项次要标准或同时满足≥3 项次要标准（表 75-3），其次再对SM 亚型进行诊断（图 75-1，图 75-2）。</p>',
  treatment:
    '<p>肥大细胞活化症状的抗介质治疗（1）避免诱因（食物、药物、过敏原、温度骤变、皮肤干燥、摩擦皮肤）；</p>',
  prognosis:
