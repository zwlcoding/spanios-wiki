...
export const diseaseCongenitalFactorViiDeficiency: DiseaseDraft = {
  ...
  medicalSections: {
    symptoms:
      '<p>主要临床表现包括：整体而言，在 CFⅦ D 中，多达 1/3 的患者无症状，在其余 2/3 患者中，大多数表现为轻度临床症状，而只有 10% 至 15% 的患者有严重出血。</p><p>不同患者不一定会出现所有表现，症状轻重也可能随年龄、病程和合并问题而变化。</p>',
    diagnosis:
      '<p>诊断通常需要把病史、体格检查、专科检查和必要的实验室/影像/病理或遗传检测放在一起判断。本病的诊断需要结合临床表现、实验室检查以及必要时的基因检测。</p><p>还需要与表现相近的疾病鉴别：共同途径凝血因子缺乏症共同途径凝血因子如 FⅤ、FⅩ、FⅡ 及纤维蛋白原均可以导致 PT 延长及 APTT 延长，而 CFⅦ D 仅有 PT 延长，通过凝血因子活检测可以鉴别。</p>',
    treatment:
      '<p>治疗应由相关专科根据疾病阶段、严重程度、受累器官和药物可及性制定。CFⅦ D 的治疗策略主要包括出血后的止血治疗、围手术期处理以预防出血、频繁/严重出血患者的预防治疗（规律替代治疗）、女性患者产后出血，以及相关并发症（反复出血导致的关节畸形、FⅦ 抑制物）的处理。</p>',
    longTermCare:
  ...
  symptoms:
    '<p>整体而言，在 CFⅦ D 中，多达 1/3 的患者无症状，在其余 2/3 患者中，大多数表现为轻度临床症状，而只有 10% 至 15% 的患者有严重出血。</p>',
  diagnosis:
    '<p>本病的诊断需要结合临床表现、实验室检查以及必要时的基因检测。</p>',
  treatment:
    '<p>CFⅦ D 的治疗策略主要包括出血后的止血治疗、围手术期处理以预防出血、频繁/严重出血患者的预防治疗（规律替代治疗）、女性患者产后出血，以及相关并发症（反复出血导致的关节畸形、FⅦ 抑制物）的处理。</p>',
  prognosis:
